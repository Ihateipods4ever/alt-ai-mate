...
const aiService_1 = __importDefault(require("./services/aiService")); // Assuming this file exists and exports an object
// Load environment variables
dotenv_1.default.config(); // Assuming dotenv.config() is still needed and works with import
...
app.use((0, cors_1.default)()); // Use the cors middleware
// --- Middleware ---
...
app.post('/api/projects', async (req, res) => {
    ...
    try {
        const result = await pool.query('INSERT INTO projects (name, project_type, user_id, created_at) VALUES ($1, $2, $3, NOW()) RETURNING *', [name, projectType, userId]);
 ...
 * @access  Public
 */
app.post('/api/generate-code', (0, cors_1.default)(), async (req, res) => {
    const { prompt, language, framework, projectType } = req.body;
    if (!prompt) {
        return res.status(400).json({ error: 'Missing prompt' });
    }
    try {
        const result = await aiService_1.default.generateCode({
            prompt,
